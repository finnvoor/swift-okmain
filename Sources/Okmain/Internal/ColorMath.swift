...
enum ColorMath {
    ...
    private static let toSRGB8Table: [UInt32] = [
    ...
    static let sRGBToLinear: [Float] = [
        ...
        0.9386859, 0.9473069, 0.9559735, 0.9646866, 0.9734455, 0.98225087, 0.9911022, 1.0,
    ]
    ...
    private static func encodeSRGB(_ linear: Float) -> UInt8 {
        let maxvBits: UInt32 = 0x3f7fffff
        let minvBits: UInt32 = 0x39000000
        var input = linear
        let minv = Float(bitPattern: minvBits)
        let maxv = Float(bitPattern: maxvBits)
        if !(input > minv) {
            input = minv
        }
        if input > maxv {
        ...
        let index = Int((bits &- minvBits) >> 20)
        let entry = toSRGB8Table[index]
        let bias = (entry >> 16) << 9
        let scale = entry & 0xffff
        let t = (bits >> 12) & 0xff
        return UInt8(((bias &+ scale &* t) >> 16) & 0xff)
    }
